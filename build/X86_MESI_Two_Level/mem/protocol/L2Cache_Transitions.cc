                                        ...
                                        int core_id)//fanyao: default invalid core id
{
  ...
  case HASH_FUN(L2Cache_State_MT_SB, L2Cache_Event_L1_PUTX_old):
    ...
    t_sendWBAck(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    jj_popL1RequestQueue(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
  ...
  case HASH_FUN(L2Cache_State_SS_MB, L2Cache_Event_L1_PUTX_old):
    zz_stallAndWaitL1RequestQueue(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(L2Cache_State_MT_SB, L2Cache_Event_MEM_Inv):
    zn_recycleResponseNetwork(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(L2Cache_State_NP, L2Cache_Event_L1_GETS):
    ...
    if (!(*m_TBEs_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    qq_allocateL2CacheBlock(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    ll_clearSharers(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    nn_addSharer(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    i_allocateTBE(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    ss_recordGetSL1ID(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    a_issueFetchToMemory(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    uu_profileMiss(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    jj_popL1RequestQueue(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(L2Cache_State_NP, L2Cache_Event_L1_GETX):
    ...
    ll_clearSharers(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    i_allocateTBE(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    xx_recordGetXL1ID(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    a_issueFetchToMemory(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    uu_profileMiss(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    jj_popL1RequestQueue(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(L2Cache_State_ISS, L2Cache_Event_Mem_Data):
    ...
    if (!(*m_responseFromL2Cache_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    m_writeDataToCache(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    ex_sendExclusiveDataToGetSRequestors(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
  ...
  case HASH_FUN(L2Cache_State_IS, L2Cache_Event_Mem_Data):
    ...
    if (!(*m_responseFromL2Cache_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    m_writeDataToCache(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    e_sendDataToGetSRequestors(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    s_deallocateTBE(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    o_popIncomingResponseQueue(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
  ...
  case HASH_FUN(L2Cache_State_IM, L2Cache_Event_Mem_Data):
    ...
    if (!(*m_responseFromL2Cache_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    m_writeDataToCache(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    ee_sendDataToGetXRequestor(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    s_deallocateTBE(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    o_popIncomingResponseQueue(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(L2Cache_State_SS, L2Cache_Event_L1_GET_INSTR):
    if (!(*m_responseFromL2Cache_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    ds_sendSharedDataToRequestor(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    nn_addSharer(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    set_setMRU(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    uu_profileHit(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    jj_popL1RequestQueue(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(L2Cache_State_SS, L2Cache_Event_L1_GETX):
    ...
    fwm_sendFwdInvToSharersMinusRequestor(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    set_setMRU(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    uu_profileHit(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    jj_popL1RequestQueue(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(L2Cache_State_SS, L2Cache_Event_L1_UPGRADE):
    ...
    if (!(*m_responseFromL2Cache_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    fwm_sendFwdInvToSharersMinusRequestor(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    ts_sendInvAckToUpgrader(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    set_setMRU(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    uu_profileHit(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    jj_popL1RequestQueue(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(L2Cache_State_SS, L2Cache_Event_L2_Replacement_clean):
    ...
    if (!(*m_TBEs_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    i_allocateTBE(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    f_sendInvToSharers(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    rr_deallocateL2CacheBlock(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(L2Cache_State_M, L2Cache_Event_L1_GETS):
    ...
    if (!(*m_responseFromL2Cache_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    dd_sendExclusiveDataToRequestor(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    set_setMRU(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    uu_profileHit(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    jj_popL1RequestQueue(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(L2Cache_State_M, L2Cache_Event_MEM_Inv):
    ...
    if (!(*m_responseFromL2Cache_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    i_allocateTBE(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    c_exclusiveReplacement(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    rr_deallocateL2CacheBlock(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(L2Cache_State_M, L2Cache_Event_L2_Replacement_clean):
    ...
    if (!(*m_responseFromL2Cache_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    i_allocateTBE(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    c_exclusiveCleanReplacement(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    rr_deallocateL2CacheBlock(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    mtit_mtitocleannotice(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(L2Cache_State_MT, L2Cache_Event_L1_GETX):
    ...
    if (!(*m_L1RequestFromL2Cache_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    b_forwardRequestToExclusive(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    uu_profileMiss(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    set_setMRU(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    jj_popL1RequestQueue(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(L2Cache_State_MT, L2Cache_Event_L1_PUTX):
    ...
    if (!(*m_responseFromL2Cache_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    ll_clearSharers(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    mr_writeDataToCacheFromRequest(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    t_sendWBAck(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    jj_popL1RequestQueue(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(L2Cache_State_MT_MB, L2Cache_Event_Exclusive_Unblock):
    ...
    mmu_markExclusiveFromUnblock(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    k_popUnblockQueue(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    kd_wakeUpDependents(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;

  case HASH_FUN(L2Cache_State_MT_IIB, L2Cache_Event_Unblock):
    next_state = L2Cache_State_MT_IB;
    nnu_addSharerFromUnblock(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    k_popUnblockQueue(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(L2Cache_State_S_I, L2Cache_Event_Ack):
    q_updateAck(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    o_popIncomingResponseQueue(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(L2Cache_State_MCT_I, L2Cache_Event_WB_Data):
    ...
    if (!(*m_responseFromL2Cache_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    qq_writeDataToTBE(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    ct_exclusiveReplacementFromTBE(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    o_popIncomingResponseQueue(m_tbe_ptr, m_cache_entry_ptr, addr, core_id);
    return TransitionResult_Valid;
